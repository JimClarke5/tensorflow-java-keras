...
public class TruePositivesTest {
    ...
    public void test_config() {
        try(TestSession session = TestSession.createTestSession(tf_mode)) {
            ...
            TruePositives instance = new TruePositives(tf, "my_tp", new float[]{0.4f, 0.9f});
            assertEquals("my_tp", instance.getName());
    ...
    public void test_weighted() {
        try(TestSession session = TestSession.createTestSession(tf_mode)) {
            ...
            session.run(update);
            Operand result = instance.result();
            session.evaluate(12.0f, instance.result());
         }
    }
    ...
    public void test_weighted_with_thresholds() {
        try(TestSession session = TestSession.createTestSession(tf_mode)) {
            ...
            session.run(update);
            Operand result = instance.result();
            Float[] expected = new Float[] {222.f, 111.f, 37.f};
            session.evaluate(expected, instance.result());
         }
    }
