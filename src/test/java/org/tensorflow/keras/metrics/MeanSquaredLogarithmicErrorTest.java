...
public class MeanSquaredLogarithmicErrorTest {
    ...
    public void testConfig() {
        try(TestSession session = TestSession.createTestSession(tf_mode)) {
            ...
            MeanSquaredLogarithmicError instance = new MeanSquaredLogarithmicError(tf,"msle", TInt32.DTYPE);
            assertEquals("msle", instance.getName());
    ...
    public void testUnweighted() {
        try(TestSession session = TestSession.createTestSession(tf_mode)) {
            ...
            session.run(op);
            Operand loss = instance.call(y_true, y_pred, null);
    ...
    public void test_weighted() {
        try(TestSession session = TestSession.createTestSession(tf_mode)) {
            ...
            Variable<TInt32> count = instance.getVariable(instance.getCountName());
            Operand result  = instance.result();
            session.evaluate(1.8257208f, total);
            session.evaluate(7, count);
            session.evaluate(0.26082f, result);
