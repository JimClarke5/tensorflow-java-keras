...
public class SpecificityAtSensitivityTest {
    ...
    public SpecificityAtSensitivityTest() {
    }
    ...
    public void testConfig() {
        try(TestSession session = TestSession.createTestSession(tf_mode)) {
            ...
            SpecificityAtSensitivity instance = new SpecificityAtSensitivity(tf,"specificity_at_sensitivity_1", 0.4f, 100 );
            assertEquals("specificity_at_sensitivity_1", instance.getName());
            assertEquals(0.4f, instance.getSensitivity());
    ...
    public void test_value_is_idempotent() {
        try(TestSession session = TestSession.createTestSession(tf_mode)) {
            Ops tf = session.getTF();
            SpecificityAtSensitivity instance = new SpecificityAtSensitivity(tf, 0.7f);
            ...
            Operand initialSpecificity = instance.result();
            
            for(int i = 0; i < 10; i++)
                session.evaluate(initialSpecificity, instance.result());
    ...
    @Test
    public void  test_unweighted_high_sensitivity() {
        try(TestSession session = TestSession.createTestSession(tf_mode)) {
            Ops tf = session.getTF();
            SpecificityAtSensitivity instance = new SpecificityAtSensitivity(tf, 0.8f);
            session.run(instance.initializeVars());
    ...
    @Test
    public void  test_unweighted_low_sensitivity() {
        try(TestSession session = TestSession.createTestSession(tf_mode)) {
            Ops tf = session.getTF();
            SpecificityAtSensitivity instance = new SpecificityAtSensitivity(tf, 0.4f);
            session.run(instance.initializeVars());
            Operand yPred = tf.constant(new float[] { 
    ...
    public void  test_invalid_sensitivity() {
        try(TestSession session = TestSession.createTestSession(tf_mode)) {
            Ops tf = session.getTF();
            SpecificityAtSensitivity instance = new SpecificityAtSensitivity(tf, -1f);
            fail();
    ...
    public void  test_invalid_num_thresholds() {
        try(TestSession session = TestSession.createTestSession(tf_mode)) {
            Ops tf = session.getTF();
            SpecificityAtSensitivity instance = new SpecificityAtSensitivity(tf, 0.4f, -1);
            fail();
        }catch(AssertionError expected) {
