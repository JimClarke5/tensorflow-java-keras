 ...
 * @author jbclarke
 */
public class TopKCategoricalAccuracyTest {
    private TestSession.Mode tf_mode = TestSession.Mode.GRAPH;
    
    public TopKCategoricalAccuracyTest() {
    }
   ...
   @Test
    public void test_config() {
        try(TestSession session = TestSession.createTestSession(tf_mode)) {
            ...
            TopKCategoricalAccuracy instance = new TopKCategoricalAccuracy(tf, "topkca", TInt32.DTYPE);
            assertEquals("topkca", instance.getName());
    ...
    public void test_correctness() {
        try(TestSession session = TestSession.createTestSession(tf_mode)) {
            Ops tf = session.getTF();
            TopKCategoricalAccuracy instance = new TopKCategoricalAccuracy(tf);
            session.run(instance.initializeVars());
            
            Operand yTrue = tf.constant(new float[][] {{0, 0, 1}, {0, 1, 0}});
            ...
            //With `k` < 5.
            instance = new TopKCategoricalAccuracy(tf, 1);
            session.run(instance.initializeVars());
    ...
    public void test_weighted() {
        try(TestSession session = TestSession.createTestSession(tf_mode)) {
            Ops tf = session.getTF();
            TopKCategoricalAccuracy instance = new TopKCategoricalAccuracy(tf,2);
            session.run(instance.initializeVars());
