...
public class TrueNegatives extends ConfusionMatrixConditionCount {
    ...
    public static final String DEFAULT_NAME = "true_negatives";
    private static final float[] DEFAULT_THRESHOLD = new float[] {0.5f};
    
    
    public TrueNegatives(Ops tf) {
        this(tf, DEFAULT_NAME, DEFAULT_THRESHOLD, null);
    }
    
    public TrueNegatives(Ops tf, float threshold) {
        this(tf, DEFAULT_NAME, new float[]{ threshold }, null);
    }
    public TrueNegatives(Ops tf, float[] thresholds) {
        this(tf, DEFAULT_NAME, thresholds, null);
    }
    
    public TrueNegatives(Ops tf, DataType dType) {
        this(tf, DEFAULT_NAME, DEFAULT_THRESHOLD, dType);
    }
    public TrueNegatives(Ops tf, String name) {
        this(tf, name, DEFAULT_THRESHOLD, null);
    }
    
    public TrueNegatives(Ops tf, String name, float threshold) {
        this(tf, name, new float[]{ threshold }, null);
    }
    
    public TrueNegatives(Ops tf, String name, float[] thresholds) {
        this(tf, name, thresholds , null);
    }
    
    public TrueNegatives(Ops tf, String name, DataType dType) {
        this(tf, name, DEFAULT_THRESHOLD, dType);
    }
    
    public TrueNegatives(Ops tf, String name,  float threshold, DataType dType) {
        this(tf, name, new float[]{ threshold}, dType);
    }
    
    public TrueNegatives(Ops tf, String name, float[] thresholds, DataType dType) {
        super(tf, name,  ConfusionMatrixEnum.TRUE_NEGATIVES, thresholds, dType);
    }
