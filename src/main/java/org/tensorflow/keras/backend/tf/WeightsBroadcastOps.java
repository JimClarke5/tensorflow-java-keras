...
public class WeightsBroadcastOps {
    ...
    private final static String ASSERT_BROADCASTABLE_ERROR_PREFIX = "weights can not be broadcast to values.";
     ...
     * @return `Operation` raising `InvalidArgumentError` if `weights` has incorrect shape.
     ...
     */
    public static <T extends TNumber > Op assertBroadcastable(Ops tf, Operand<T> weights, Operand<T> values) {
            ...
            if(weightsRankStatic != valuesRankStatic) {
                throw new IllegalArgumentException(
                    String.format("", 
                        ...
                        ASSERT_BROADCASTABLE_ERROR_PREFIX, valuesRankStatic, weightsRankStatic, 
                        valuesShapeStatic.toString(), weightsShapeStatic.toString() )
                );
            }
                ...
                if(valuesShapeStatic.size(i) != weightsShapeStatic.size(i)) {
                    throw new IllegalArgumentException(
                        String.format("", 
                            ...
                            ASSERT_BROADCASTABLE_ERROR_PREFIX, i, 
                            valuesShapeStatic.toString(), weightsShapeStatic.toString() )
                    );
                }
            }
        ...
        List<Operand<?>> data = Arrays.asList(
                tf.constant(ASSERT_BROADCASTABLE_ERROR_PREFIX),
                tf.constant("weights.shape="), weightsShape,
                tf.constant("values.shape="),  valuesShape,
                tf.constant("is_scalar="), is_scalar
        );
    ...
    private static <T extends TNumber > Operand<TBool> hasValidNonscalarShape(
            Ops tf, Operand<T> weightsRank, Operand<T> weightsShape, 
            Operand<T> valuesRank, Operand<T> valuesShape) {
     ...
     */
    public static <T extends TNumber> Operand<T> broadcastWeights(Ops tf, Operand<T> weights, Operand<T> values) {
